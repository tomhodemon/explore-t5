...
class T5Block(nn.Module, ABC):
    ...
    @abstractmethod
    def apply_ffn(self, hidden_states):
        pass
...
class T5BlockSharedFFN(T5Block):
    def __init__(self, sharedFFN, config, has_relative_attention_bias=False):
        super().__init__(config, has_relative_attention_bias)
        self.sharedFFN = sharedFFN

    def apply_ffn(self, hidden_states):
        return self.sharedFFN(hidden_states)

    
class T5Stack(nn.Module, ModuleUtilsMixin):
    def __init__(self, config, embed_tokens, sharedFFN):
        super().__init__()
        ...
        if sharedFFN:
            sharedFFN = T5LayerFF(config)
            self.block = nn.ModuleList(
                [T5BlockSharedFFN(sharedFFN, config, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
            )
        else:
            self.block = nn.ModuleList(
                [T5BlockVanilla(config, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
            )
...
class MyT5(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.encoder = T5Stack(encoder_config, self.shared, config.sharedEnc)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack(decoder_config, self.shared, config.sharedDec)
